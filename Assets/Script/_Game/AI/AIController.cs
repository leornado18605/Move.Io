...
public class AIFollowPlayer : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (currentTarget != null) SetState(AIState.Move);
    }
    ...
    public string GetAIName()
    {
        if (nameTag != null)
        {
            var nameTagComp = nameTag.GetComponent<AINameTag>();
            if (nameTagComp != null)
            {
                return nameTagComp.GetName();
            }
        }
        return "AI_" + GetInstanceID();
    }

    public int GetScore()
    {
        return nameTag != null ? nameTag.GetComponent<AINameTag>().GetScore() : killCount;
    }
}